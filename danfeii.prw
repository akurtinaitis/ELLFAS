			...
			For nX := 1 To nLenNotas
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						// ELLFAS 
                        // FUNCAO ESPECIFICA PARA IMPRESSAO DA GUIA DE GNRE 
						ImpGNRE(@oDanfe,aNotas[nX][02],aNotas[nX][05],aNotas[nX][04])							
...
//-----------------------------------------------------------------------
/*/{Protheus.doc} ImpGNRE
Impressao e tansmissao da GNRE especifico ELLFAS.
 
@author 	Montes Wikitec
@since 		21/04/2019
@version 	12.1.17
 
@param	oDanfe, cTipoMov, cNFiscal, cSerie

@return Nil
/*/
//-----------------------------------------------------------------------
Static Function ImpGNRE(oDanfe,cTipoMov,cNfiscal,cSerie)

Local cChaveSFT := (xFilial("SFT")+cTipoMov+cNfiscal+cSerie)
Local cChaveSF6 := ""                                                  
Local lEnd      := .F.
Local aArea     := GetArea()
Local nRecSF6   := Nil
...
If dbSeek(cChaveSFT)
   dbSelectArea("SF6")
   dbSetOrder(3) //F6_FILIAL+F6_OPERNF+F6_TIPODOC+F6_DOC+F6_SERIE+F6_CLIFOR+F6_LOJA
   cChaveSF6 := xFilial("SF6")+IIF(SFT->FT_TIPOMOV=="S","2","1")+IIF(EMPTY(SFT->FT_TIPO),"N",SFT->FT_TIPO)+SFT->FT_NFISCAL+SFT->FT_SERIE+SFT->FT_CLIEFOR+SFT->FT_LOJA
   If dbSeek(cChaveSF6)                       
	  While SF6->(!EOF()) .And. SF6->(F6_FILIAL+F6_OPERNF+F6_TIPODOC+F6_DOC+F6_SERIE+F6_CLIFOR+F6_LOJA) == cChaveSF6
		 
		 If SF6->F6_TIPOIMP $ "3-B"  //ICMS ST e DIFAL #FECP DIFAL            
			If EMPTY(SF6->F6_CDBARRA)
						   				
			   //Transmite GNRE         
			   cAmbGNRE := alltrim(GetNewPar("MV_AMBGNRE","2")) 
			   cURLGNRE := Padr(GetNewPar("MV_SPEDURL","http://"),250)                              
			   aRetErro := {}                                       
			   cNumGNRE := SF6->F6_NUMERO
			   cUF      := SF6->F6_EST
			   nRecSF6  := SF6->(RECNO())
	 
			   StaticCall( FISA095, GnreTrans, cNumGNRE, cNumGNRE, cAmbGNRE, lEnd )   
			   For x := 1 To 20 step 1 
				   
				   Inkey(1)
				   
				   StaticCall( FISA095, getListBox, cURLGNRE, cNumGNRE, cNumGNRE, cAmbGNRE, aRetErro )					   		        
			...
			If !EMPTY(SF6->F6_CDBARRA) 
			   cAutoriza   := SF6->F6_CDBARRA
			   cXMLENVGNRE := SF6->F6_XMLENV
			   cNumCtrl    := SF6->F6_NUMCTRL
			   StaticCall( FISA119, ImpDetGnre, @oDanfe, cAutoriza, cXMLENVGNRE, cNumCtrl )						                
			Else
			   Aviso("GNRE","GNRE de "+cUF+" n?o autorizada! Verifique mensagem na monitor de transmiss?o de GNRE.",{"Ok"},3)
			EndIf
		 EndIf   
