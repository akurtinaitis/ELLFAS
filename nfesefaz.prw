...
Local cxTipoNF  := "" // ELLFAS
Local cxPedido  := "" // ELLFAS
Local cxItemPC  := "" // ELLFAS
Local nBsIcm60  := 0  // ELLFAS
Local nVlIcm60  := 0  // ELLFAS
...
// ELLFAS
Private cEmailVend := ""  //Email do vendedor 
...
IF Alltrim(GetNewPar("MV_CMPCNPJ",""))  <>  ""
	aCampoCnpj := StrTokArr( GetNewPar("MV_CMPCNPJ",""), "|" )	
Endif 
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				If !Empty(SF2->F2_TRANSP)
					...
					aadd(aTransp,AllTrim(SA4->A4_NOME)+" - "+AllTrim(SA4->A4_TEL))
					...
					aadd(aTransp,SA4->A4_EMAIL	)
				   	...
				   	If len(aCampoCnpj) > 0 .and. !Empty(SA4->A4_CGC) .and. ASCAN(aCampoCnpj, { |x| UPPER(x) == "C5_TRANSP" }) > 0
				...
				If MsSeek(cChave)
					...
					If lSimpNac
						...
						Else
							...
							If SF2->F2_VALICM > 0 .And. !Alltrim(SF3->F3_CFO) $ cCfop .And. EMPTY(SF2->F2_ICMSRET)
								cMensFis += "Documento emitido por ME ou EPP optante pelo Simples Nacional."
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
					    ...
					    EndIf
						If !AllTrim(SC5->C5_MENNOT1) $ cMensCli
						   If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
						      cMensCli += " "
						   EndIf
						   cMensCli += " | " + AllTrim(SC5->C5_MENNOT1)
					    EndIf
					    If !AllTrim(SC5->C5_MENNOT2) $ cMensCli
						   If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
							  cMensCli += " "
						   EndIf
						...
						EndIf
                        // ELLFAS - INICIO
				        If SF4->(FIELDPOS("F4_MSGNOT")) > 0
				           If !Empty(SF4->F4_MSGNOT) .And. !AllTrim(SF4->F4_MSGNOT) $ cMensFis
				              If Len(cMensFis) > 0
						         cMensFis += " | "
					          EndIf
					          cMensFis += AllTrim(SF4->F4_MSGNOT)
				           EndIf
				        EndIf	 

				        If !EMPTY(SF2->F2_VEND1)      
				           cEmailVend := Posicione("SA3",1,xFilial("SA3")+SF2->F2_VEND1,"A3_EMAIL")
				        Else
				           cEmailVend := ""
				        EndIf				    
				        If !EMPTY(SF2->F2_VEND2)      
				           cEmailVend += IIF(!EMPTY(cEmailVend),";","")+Posicione("SA3",1,xFilial("SA3")+SF2->F2_VEND2,"A3_EMAIL")
				        Else
				           cEmailVend += ""
				        EndIf				    
				        // ELLFAS - FIM
                        ...
                        IF len(aCampoCnpj) > 0  
							...
							For nX := 1 To Len(aCampoCnpj)
								...
								If !Empty(aCampoCnpj[nX]) .and. "C5_" == substr(alltrim(aCampoCnpj[nX]),1,3) .and. SC5->(ColumnPos(aCampoCnpj[nX])) > 0 .and. !Empty(&("SC5->"+aCampoCnpj[nX])) 

                                	If aCampoCnpj[nX] == "C5_REDESP"
										dbSelectArea("SA4")
										dbSetOrder(1)

										If MsSeek(xFilial("SA4")+SC5->C5_REDESP)
											cCnpjPart := AllTrim(SA4->A4_CGC)
                        				EndIf
									else
										cCnpjPart := alltrim(&("SC5->"+aCampoCnpj[nX]))
									endif
				...
				If Len(aIPIDev)>0
					...
					EndDo
                        ...
                        // ELLFAS
			            If nBsIcm60 > 0 // Devolução de Compra com ICMS ST
			               cMensCli += " "
			               cMensCli += "(Base de Calculo do ICMS: R$ "+AllTrim(Str(nBsIcm60,15,2))+ " - "+"Valor do ICMS: R$ "+AllTrim(Str(nVlIcm60,15,2))+") "
			               nBsIcm60 := 0
			               nVlIcm60 := 0
                        EndIf					
				EndIf
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				if FindFunction( "SpedNatOper" ) .And. SFT->( MsSeek( xFilial( "SFT" ) + cChaveD1 ) )
					...
					Else
                        // ELLFAS  
					    If SF1->F1_TIPO = "C" .And. SF1->F1_EST = "EX"   
				           cNatOper := SF4->F4_TEXTO
				        Else
						   aSX513 	 := FwGetSX5("13",SF4->F4_CF)
						   If len(aSX513) > 0
				...
				If !Empty((cAliasSD1)->D1_NFORI)
					...
					If !(cAliasSD1)->D1_TIPO $ "DBN" .Or. lRural
						...
						if !lRural .and. lSeekOk
                            ...
                            // ELLFAS - INICIO
                            cxTipoNF:= SD1->D1_TIPO_NF
							cxPedido:= SD1->D1_PEDIDO
							cxItemPC:= SD1->D1_ITEMPC	
                            // ELLFAS - FIM  
				...
				If SF4->(FIELDPOS("F4_MSGNOT")) > 0
				   ...
				   EndIf
				EndIf
				If SF1->F1_TIPO = "D" .And. Len(aNfVinc) > 0 // Dev.NF Complemento de ICMS
                   cMensX := "DEVOLUCAO REF. NF No."+aNfVinc[1][3]+" "+aNfVinc[1][2]+" de "+DTOC(aNfVinc[1][1])+"."
                   If !cMensx $ cMensFis
                      If Len(cMensFis) > 0
                         cMensFis += " | "
                      EndIf                                                                                            
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					nDescRed + nDescIcm + nDescNfDup + (cAliasSD1)->D1_VALDESC + nDescFis -(SFT->FT_DESCZFR),;
				   	RetPrvUnit(cAliasSD2,nDesconto),; // ELLFAS IIF(!((cAliasSD1)->D1_TIPO $"IP"),(cAliasSD1)->D1_VUNIT,0),;  
				   	IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF  
				...
				If lEasy  .And. ( !Empty((cAliasSD1)->D1_TIPO_NF) .Or. ( Empty((cAliasSD1)->D1_TIPO_NF) .And. (cAliasSD1)->D1_TIPO $ 'IPC' )   )
					...
					Else
						...
						// aadd(aDI,(GetNFEIMP(.F.,cNFOri,cSerOri,cFornece,cLojaEnt,cTipoNFEnt, ,cItemOri, ,cItem )))
						   aadd(aDI,(GetNFEIMP(.F.,cNFOri,cSerOri,cFornece,cLojaEnt,cxTipoNF,cxPedido,cxItemPC)))  
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
    ...
    // ELLFAS
	If !EMPTY(cEmailVend) //Inclusão de email do vendedor 
		If !Empty(AllTrim(aDest[16])+cMailTrans)
			cEmailVend := ";"+AllTrim(cEmailVend)
		Else 
			cEmailVend := AllTrim(cEmailVend)
		EndIf
	Else                  
	...
	if !lBrinde
		...
		cString += NfeTag('<EMAIL>',AllTrim(aDest[16])+cMailTrans+cEmailVend)
